      ...
      m_alias(std::make_shared<Alias>())
{
  m_dispatchTable = {{"exit",
                     ...
                     {"createalias", [&](std::vector<char *> &tokens)
                      { return m_alias->create(tokens); }},

                     {"destoryalias", [&](std::vector<char *> &tokens)
                      { return m_alias->destroy(tokens); }}};
...
int Prompt::main()
{
  ...
  while(true)
  {
    ...
    for(auto &line : lines)
    {
      ...
      if(m_alias->isAlias(lineTokens[0]) && lineTokens.size() == 2)
      { // Alias can only be the first token
